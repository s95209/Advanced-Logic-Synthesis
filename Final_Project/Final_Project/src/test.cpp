...
std::string executeCommand(const std::string &command, const std::string &filename) {
    ...
    std::string redirectCommand = command + " > " + tempFileName + " 2>&1";
        ...
        // while (uphill < currentEncoding.size() * 20 || swap_solution_count < currentEncoding.size() * 40) {
            ...
            if ( newCost > 100 && ((newCost < currentCost )|| std::exp((currentCost - newCost) / temperature) > std::generate_canonical<double, 10>(rng)) ) {
                ...
                if (newCost < bestCost) {
                    ...
                    for (const auto &state : bestEncoding) {
                        costFile << state.first << ": " << state.second << '\n';
                    }
                }
            } else {
                // Undo the move
...
int main(int argc, char **argv) {
    ...
    std::string cost_Filename = Filename + "_cost.txt";
    std::ofstream costFile(cost_Filename);
    if (!file.is_open()) {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Simulated Annealing
    double initialTemperature = 1000;  // Initial temperature
